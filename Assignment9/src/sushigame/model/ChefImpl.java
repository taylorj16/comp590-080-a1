	...
	public void handleBeltEvent(BeltEvent e) {
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				...
				int size = plate.getContents().getIngredients().length;
				IngredientPortion[] ingrPor = new IngredientPortion[size];
				ingrPor = plate.getContents().getIngredients();
				for(int i = 0; i< ingrPor.length; i++) {
					amountConsumed += ingrPor[i].getAmount();
				}
			...
			if (plate.getChef() == this) {
				...
				for(int i = 0; i< ingrPor.length; i++) {
					amountSpoiled += ingrPor[i].getAmount();
				}
				Customer consumer = belt.getCustomerAtPosition(((PlateEvent) e).getPosition());
				plate_history.add(new HistoricalPlateImpl(plate, consumer));
			}
