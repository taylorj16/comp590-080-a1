...
public class PlayerChefController implements ChefViewListener {
	...
	private void placePlate(Plate plate, int position) {
		try {
			...
			game_view.setControllerMessage("Already placed a plate this rotation. Rotate and remake if Roll.");
		} catch (Exception e) {
	...
	public void handleRedPlateRequest(Sushi plate_sushi, int plate_position) {
		try {
			...
			game_view.setControllerMessage("Sushi too costly for red plate. Remake if Roll.");
		}
	}
	...
	public void handleGreenPlateRequest(Sushi plate_sushi, int plate_position) {
		try {
			...
			game_view.setControllerMessage("Sushi too costly for green plate. Remake if Roll.");
		}
	}
	...
	public void handleBluePlateRequest(Sushi plate_sushi, int plate_position) {
		try {
			...
			game_view.setControllerMessage("Sushi too costly for blue plate. Remake if Roll.");
		}
	}
	...
	public void handleGoldPlateRequest(Sushi plate_sushi, int plate_position, double price) {
		try {
			...
			game_view.setControllerMessage("Sushi too costly for gold plate. Remake if Roll.");
		}
	}
